...
void Main(){
    ...
    Rect goal_cell{630, 470, 10, 10};
    Rect jewel_cell{630, 490, 10, 10};
    Rect player_cell{630, 510, 10, 10};
    Rect fire_cell{630, 530, 10, 10};
    ...
    if(cnt != Q){
        Print << U"Invalid output : too small number of actions";
        invalid = true;
    }
    /*
    if(not player.back().escaped){
        Print << U"Failed to escape from the labyrinth";
        invalid = true;
    }
    */
    /*
    AnimatedGIFWriter gifWriter;
    gifWriter.open(U"vis.gif", grid_size * 5, grid_size * 5);
    for(auto g : fields){
        Image img(grid_size, grid_size, Palette::Black);
        CopyToImage(g, img);
        img.fit(grid_size * 5, grid_size * 5);
    ...
    */
    ...
    while (System::Update()){
        ...
        font(U" : 扉").draw(650, 460);
        jewel_cell.draw(Palette::Skyblue);
        font(U" : 宝石").draw(650, 480);
        player_cell.draw(Palette::Lime);
        font(U" : プレイヤー").draw(650, 500);
        fire_cell.draw(Palette::Orange);
        font(U" : 火薬").draw(650, 520);
        ...
        // 画像の更新
        if (updated and (not invalid) and (not player[turn].escaped)){
        ...
        if(Rect{0, 0, 599}.mouseOver()){
            ...
            font(U"(", cursor_y, U", ", cursor_x, U")").draw(620, 550);
            if(fields[turn][cursor_y][cursor_x].status == cell_status::enemy){
                font(U"d = ", enemy[enemy_number[cursor_y][cursor_x]].d).draw(720, 550);
            }
        }
